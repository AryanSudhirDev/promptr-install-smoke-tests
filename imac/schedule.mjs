...
// The ceiling is a typo guard for the job ledger, not a volume policy. Achievable throughput is far
// lower (about 14,000 checks/day for the whole fleet at three concurrent checks averaging 18.5s), and
// the daily targets are deliberately not clamped to it. See "Daily QA volume policy" in OPERATIONS.md.
export const MAX_TOTAL = 50000;
export const validTotal = n => Number.isInteger(n) && n >= 1 && n <= MAX_TOTAL;
export const slotAt = ms => Math.floor((ms - OFFSET) / SLOT_MS) * SLOT_MS + OFFSET;
export function countForSlot(ms, total) {
  if (!validTotal(total)) throw new Error(`daily total must be 1-${MAX_TOTAL}`);
