...
// `home` is observed whether or not it is required, because it decides the lane count even when
// it is not a condition for running at all: at home the laptop runs high concurrency, away it
// keeps going on DAY_WORKERS. `atHome` checks the subnet first and only reaches for SSH on a
// match, so probing while away stays cheap.
export async function eligibility(settings,{configFresh=true,probeHome=true,knownHome=false}={}){validateSettings(settings);const p=await power();const home=probeHome?await atHome():knownHome;return {...gate({settings,power:p,home,freeBytes:freeBytes(),configFresh}),power:p,home};}
export function workerStatus(update){atomic(path.join(ROOT,'worker-status.json'),{at:new Date().toISOString(),...update});}
export async function broker(action,input,settings,{signal,timeout=120000}={}){if(!['peek','claim','complete','recover','macbook-peek','macbook-claim','macbook-complete','macbook-recover'].includes(action))throw new Error('Invalid broker action');const c=localConfig(),cmd='export PATH="$HOME/.local/bin:$HOME/.local/lima/bin:/usr/bin:/bin:/usr/sbin:/sbin"; exec '+[c.remoteNode,c.brokerPath,action].map(shellQuote).join(' ');const r=await run('/usr/bin/ssh',[...sshArgs(settings.requireHome),cmd],{timeout,signal,input:input?JSON.stringify(input):'',maxBuffer:32*1024*1024});let parsed;try{parsed=JSON.parse(r.stdout);}catch{throw new Error('Invalid broker response');}if(parsed.ok===false)throw Object.assign(new Error(parsed.error||'Broker rejected request'),{code:parsed.code,retryAfterMs:parsed.retryAfterMs});return parsed;}
