...
fs.writeFileSync(plist,`<?xml version="1.0" encoding="UTF-8"?>\n<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n<plist version="1.0"><dict><key>Label</key><string>${label}</string><key>ProgramArguments</key><array><string>${xml(fs.existsSync('/opt/homebrew/bin/node')?'/opt/homebrew/bin/node':process.execPath)}</string><string>${xml(path.join(runtime,'macbook/supervisor.mjs'))}</string></array><key>RunAtLoad</key><true/><key>KeepAlive</key><true/><key>ThrottleInterval</key><integer>30</integer><key>ProcessType</key><string>Background</string><key>Nice</key><integer>10</integer><key>LowPriorityIO</key><true/><key>Umask</key><integer>63</integer><key>EnvironmentVariables</key><dict><key>HOME</key><string>${xml(process.env.HOME)}</string><key>MACBOOK_QA_HOME</key><string>${xml(root)}</string><key>PATH</key><string>/Applications/Docker.app/Contents/Resources/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin</string></dict><key>StandardOutPath</key><string>${xml(path.join(root,'supervisor.log'))}</string><key>StandardErrorPath</key><string>${xml(path.join(root,'supervisor-error.log'))}</string></dict></plist>\n`,{mode:0o600});
await run('/usr/bin/plutil',['-lint',plist]);const domain='gui/'+process.getuid();
if(options.load==='false'){
 console.log('Service files prepared and plist validated. Load the login agent from your own Terminal.');
 try{const attributes=await run('/usr/bin/xattr',['-l',plist]);if(attributes.stdout.includes('com.apple.quarantine:'))console.log('macOS marked this generated plist as quarantined. See macbook/README.md for the narrow, single-file approval step before loading.');}catch{}
 process.exit(0);
}
