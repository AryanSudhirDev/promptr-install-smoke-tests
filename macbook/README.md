...
The setup command accepts `--load false` to prepare/validate files when running from a sandbox that cannot invoke launchd. macOS 27 also refuses quarantined LaunchAgent plists. Files generated by Aside can carry that attribute even when their syntax and ownership are correct. For this reviewed, newly generated service file only, clear that flag if present, then load the agent in the user's normal Terminal:

```sh
plist="$HOME/Library/LaunchAgents/com.promptr.qa.macbook.plist"
xattr -p com.apple.quarantine "$plist" >/dev/null 2>&1 && xattr -d com.apple.quarantine "$plist"
launchctl bootstrap gui/$(id -u) "$plist"
```

Do not clear quarantine recursively from other apps or turn off Gatekeeper. Error 5 is generic: check the specific launchd log entry rather than assuming every occurrence is the same cause. The initial Terminal failure here explicitly reported a quarantined plist; Aside's own attempts separately reported a forbidden bootstrap caller.

The runtime includes Docker Desktop's `Contents/Resources/bin` in its child-process PATH on macOS, so Docker can locate `docker-credential-desktop` without altering the user's credential store. On first eligible startup it opens the installed Docker Desktop and builds the native image. If Docker has unfinished first-run setup, its local status identifies that blocker; the service cannot accept permissions or licensing prompts for the user.
