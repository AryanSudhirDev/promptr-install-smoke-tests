...
import {
 DAY_WORKERS,OVERNIGHT_HOURS_DEFAULT,OVERNIGHT_MAX_WORKERS,RECOMMENDED_DOCKER_GIB,applyOvernight,dockerGiB,
 localStatusAllowed,overnightClaimPlan,overnightNightChecks,parseOvernightInput,readOvernight,workerCount,
} from './overnight.mjs';
...
function overnightCopy(s){
 ...
 return `Daytime uses ${s.dayWorkers} concurrent checks. An 8-hour night at ${s.recommendedDockerGiB} GiB is about ${s.fullNightChecks.toLocaleString('en-US')} checks (~${s.fullNightPerHour}/hour) across ${s.overnightMaxWorkers} slots. RAM could compute ~${s.fullNightComputePerHour}/hour; Open VSX pacing is the slower part.`;
}
