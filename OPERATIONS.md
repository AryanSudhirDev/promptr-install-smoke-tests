...
Owner instruction 2026-09-15, applied the same day: the MacBook's high-concurrency mode is **no longer night-only**. It runs all day while the laptop is plugged in and on the home network, and the nightly "going to sleep" button now runs until **7:50 AM local** instead of a fixed eight hours. `OVERNIGHT_HOURS_MAX` is 24, and the supervisor renews a 24-hour window (`SUSTAINED_HOURS`) on each poll whenever `shouldSustain` sees `enabled && onAC && home`; losing AC or leaving the home network ends it and the laptop falls back to `DAY_WORKERS: 3`. `macbook-config.json` therefore has `requireAC: true` and `requireHome: true`, which is what makes "plugged in and at home" a hard gate rather than advice. This does not change any planned total: a 24-hour window scales to the same 10,500 Promptr + 18,000 CogniSpec MacBook plan an 8-hour night already produced, and it does not touch the iMac's three containers, the eleven-lease MacBook cap, the two Open VSX streams or the 650 ms per-stream gap. It buys availability, not rate.
