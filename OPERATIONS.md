...
- Daily volume is owner-set and is not an agent's call. The fleet plans 10000 checks/day: 6000
  Promptr + 4000 CogniSpec overall (iMac 3000+2000, MacBook 3000+2000). Never lower these values,
  add a tighter clamp, or raise them back toward the retired 4000/6000 and 2000/3000 on your own
...
### Two streams at 2.5 req/s (2026-09-15)

Owner instruction: use more of the published Open VSX free-tier budget (`< 3` req/s per public IP) without camping on that ceiling. The limiter now has **two** in-flight streams. Each stream still waits **650 ms after its own response body**. Starts across both streams are at least **400 ms** apart (**2.5 req/s**). Shared 429/503 cooldown and host allow-list are unchanged. Fair-share turns are **MacBook, MacBook, MacBook, iMac** when both are waiting: the laptop converts a download into a finished check faster and can run more testers. If only the iMac is queued, it still takes every turn. The iMac still runs at most three test containers. Do not add a third stream, a hotspot broker, or a second limiter. Existing `version: 1` state files are read and rewritten as `version: 2` on the next persist. Stream completions serialize `state.json` through a persist lock and unique temp files so two processes cannot clobber `state.json.tmp`.
...
Owner-set targets, current as of this entry: the iMac plans `imacDailyTotal: 4000` Promptr and `cognispecDailyTotal: 6000` CogniSpec checks/day in `monitor-config.json`, for 10,000 planned fresh-install checks/day. Colima stays at three containers; 10,000/day is about 70% of a full 24-hour three-wide duty cycle at 18.5s/check, leaving slack for slow batches. The MacBook's independent daytime plan is 2000 Promptr + 3000 CogniSpec in `macbook-config.json`; overnight can raise the MacBook-only scheduler totals without changing the iMac.

Owner instruction 2026-09-15, applied the same day: the two extensions' shares were **swapped** so CogniSpec now carries the larger plan. Every planned total moved to the other extension and no combined total changed: iMac 6000/4000 became 4000 Promptr + 6000 CogniSpec, the MacBook daytime plan 3000/2000 became 2000 Promptr + 3000 CogniSpec, and the overnight night plan 6000/3500 became `OVERNIGHT_PROMPTR_CHECKS: 3500` + `OVERNIGHT_COGNISPEC_CHECKS: 6000` in `macbook/overnight.mjs`. Promptr's per-extension number is lower than it was **by owner instruction**; that is not a volume reduction to undo and not the retired "low hundreds per day" rule reasserting itself. Do not "restore" Promptr to the larger share on your own initiative.

Owner instruction 2026-09-15, applied the same day: the MacBook's high-concurrency mode is **no longer night-only**. It runs all day, and the nightly "going to sleep" button now runs until **7:50 AM local** instead of a fixed eight hours. `OVERNIGHT_HOURS_MAX` is 24 and the supervisor renews a 24-hour window (`SUSTAINED_HOURS`) on each poll whenever `shouldSustain` is satisfied; otherwise the laptop falls back to `DAY_WORKERS: 3`.

The condition for **high concurrency** is the home network plus power the gate accepts. Power is decided in one place, `powerAllowsWork` in `macbook/policy.mjs`: on AC it always passes, and on battery only at or above `minBatteryPercent`, which is **40**. So below 40% the laptop must be plugged in, and at 40% or above it may run on battery. The floor deliberately does not apply on AC, because a low charge while plugged in is filling up rather than draining; that is why `gate` consults `powerAllowsWork` instead of rejecting a low percentage outright. `requireAC` still exists and still forces mains power if a dashboard turns it on.

Being at home is **not** a condition for running at all. Owner instruction, same day: away from home the laptop keeps working at `DAY_WORKERS: 3` rather than stopping, so `macbook-config.json` has `requireHome: false` and `requireAC: false`. Home therefore decides the *lane count*, not eligibility, and two consequences follow. First, `eligibility` in `macbook/runtime.mjs` probes `atHome()` unconditionally instead of only when `requireHome` is set, because the observed value is needed even when it is not required; that stays cheap because `atHome` checks the subnet before trying SSH. Second, `maintainSustainedWindow` **removes** an active window as soon as `shouldSustain` stops holding, since a lane count is fixed when the worker starts and leaving the house would otherwise hold eleven lanes until the window expired.
...
**Current numbers, superseding every figure above: 10000 checks/day.** iMac `imacDailyTotal: 3000` + `cognispecDailyTotal: 2000` in `monitor-config.json`; MacBook 3000 + 2000 in `macbook-config.json` and in `DEFAULT_SETTINGS`; `OVERNIGHT_PROMPTR_CHECKS: 1000` + `OVERNIGHT_COGNISPEC_CHECKS: 666` in `macbook/overnight.mjs`. The overnight constants are the eight-hour figures `overnightNightChecks` scales by window length; they are chosen so a window of any length lands back on the 3000/2000 plan instead of raising it (`1000*24/8 = 3000`; `666*24/8 = 1998`, then `max(2000, ceil(1998))`). High concurrency still does not change volume — it only spends the same MacBook plan faster.

Owner instruction 2026-09-17: after the MacBook was home on eleven lanes with 0% 503s under load (352 passed / 30 min), step the fleet 8000 → 10000 as 6000 Promptr / 4000 CogniSpec (iMac 3000+2000, MacBook 3000+2000). Promptr stays the larger share. This is a watch-and-step, not a restore of 22,500/day. If 503s climb toward 15–30% or the cooldown latches for hours, do not raise further.
...
Applied on owner instruction 2026-09-16: volume to 6000/day, and the `Cache-Control: no-cache` header removed from `imac/registry-fetch.cjs` (guarded by a test in `tests/imac-registry.test.mjs`). Owner instruction 2026-09-17: stop watching for another volume step and lower 503s instead. The limiter now stretches cooldown exponentially on consecutive 429/503s (Retry-After, then 2×, 4×, … capped at five minutes) and allows only one in-flight probe until a non-throttle response; a sibling that wins a stream after the first 503 is already recorded does not send. Two streams at 2.5 req/s and the 650 ms per-stream gap are unchanged once the registry answers. Retries on the preparation path (`maxAttempts: 1`) and per-extension cooldown scoping stay declined: one 503 still fails that job, and a CogniSpec 503 still pauses Promptr. Reducing volume further is not the first thing to reach for.
...
What bounds the load on Open VSX is not the daily total; it is the shared limiter and the concurrency cap. Owner instruction 2026-09-15: two in-flight streams, 650 ms after each stream's own response body, and 400 ms between starts (2.5 req/s), still under the published community `< 3` rps ceiling. Do not add a third stream or change the 650 ms per-stream gap. At most three QA checks run at once, shared between the iMac and the MacBook rather than three per machine. At five wire requests per check (metadata, checksum, VSIX, plus redirects), 10,000 iMac checks/day is about 50,000 requests/day, near 0.58 requests/second averaged over the day, well under 2.5. At 15–18.5 seconds per check, it is about eight to nine hours of three-container work spread across the 288 five-minute slots, about 17 checks per slot (7 Promptr, 10 CogniSpec after the swap); the first slot at the new rate finished 17/17 well inside its five minutes.

The volume buys sample count per unit time, which is the whole point of this monitor. Each check is an independent end-to-end sample of the publish-to-install path: registry metadata, the published SHA-256, the artifact bytes, a clean VS Code install with empty user and extension directories, activation, commands, settings and keybinding. Split across six variants and two extensions, 4000/6000 gives each Promptr variant about 670 runs/day and each CogniSpec variant about 1,000, so a regression confined to one variant surfaces within a slot or two instead of the next day. At the old ~100/day rate a single-variant failure could hide for hours while users hit it immediately.
