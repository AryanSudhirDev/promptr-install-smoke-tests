...
- Daily volume is owner-set and is not an agent's call. The iMac plans 6000 Promptr + 4000
  CogniSpec checks/day and the MacBook's saved daytime plan is 3000 Promptr + 2000 CogniSpec. Never
...
Owner-set targets, current as of this entry: the iMac plans `imacDailyTotal: 6000` Promptr and `cognispecDailyTotal: 4000` CogniSpec checks/day in `monitor-config.json`, for 10,000 planned fresh-install checks/day. Colima stays at three containers; 10,000/day is about 70% of a full 24-hour three-wide duty cycle at 18.5s/check, leaving slack for slow batches. The MacBook's independent daytime plan remains 3000 Promptr + 2000 CogniSpec in `macbook-config.json`; overnight can raise the MacBook-only scheduler totals without changing the iMac.
