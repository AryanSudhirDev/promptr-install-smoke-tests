...
The iMac had only about 7.6 GiB of disk free at setup, despite reasonable CPU/memory headroom. Before each new check, the runner now verifies at least 5 GiB of available disk. Below that floor it pauses new work and leaves pending jobs queued under the existing six-hour catch-up window. It does not delete user files or kill active checks. This safeguard is not a substitute for freeing disk space. As of 2026-09-17 the monitor also drops passed-job report folders after the ledger write and, once free space is below about 6 GiB, prunes completed leftovers and archives before that 5 GiB pause; in-flight and failed evidence is kept until that prune. Checks still download, hash, install and assert.
